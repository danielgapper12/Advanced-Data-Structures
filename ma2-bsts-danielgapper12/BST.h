...
public:
	...
	BST(const BST& other) : _root(nullptr) {
		...
		// Clone from the root of other tree
		_root = cloneTree(other._root); // my solution 
	}
	...
	BST(BST&& other) : _root(nullptr) {
		...
		_root = other._root;
		other._root = nullptr; // my solution, makes sure other is empty
	}
	...
	BST& operator=(BST& other) {
		...
		if (this != &other) // my solution
		{
			this->makeEmpty(); // clears current tree to prevent memory leaks
	...
	BST& operator=(BST&& other) {
		...
		if (this != &other) // solution
		{
			makeEmpty();

			// move ownership of root
